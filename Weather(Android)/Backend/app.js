...
function getGoogleObj2(addr, res) {
  var url = googlePath + addr + '&key=AIzaSyAwr8GaCxXJ70WJtLwTK1YSvnlew1-TbSs';
  url = encodeURI(url);
  var e = request(url, function(error,response,body){
        if(!error && response.statusCode==200){
          googlePos = JSON.parse(body);
...
app.get('/weatherSearch2', (req, res, next) => {
  var body = req.query;
  getGoogleObj2(body.addr, res);
})
...
app.get('/photoSearch', (req, res, next) => {
  var body = req.query;
  var url = customPath+'q='+body.q+
  '&cx=013849470689090416768:hixbvoykocs&filter=1&imgSize=xxlarge&imgType=photo&num=8&searchType=image&key=AIzaSyAoJ7sQBpmIRKdCf2Ch7Ri9Wu6UMAk_RnI';
  url = encodeURI(url);
  var e = request(url, function(error,response,body){
   if(!error && response.statusCode==200){
     res.json(JSON.parse(body));
...
app.get('/', (req, res, next) => {
  res.send("Backend for Homework 9 Android project");
})
